...
columnId || (columnId = "MANU228065"); //央视点播栏目ID   MANU228065
var nav = [];
nav[0] = columnId; //
...
function getZiyuan(dataJson) { //用于首页
    ...
    //  $('b1').innerText=str;
    var arr = ['', '', ''];
    $('showMidd1').getElementsByTagName('img')[0].src = gPoster(data[4].imageList, 360, 270);
    $('showMidd2').getElementsByTagName('img')[0].src = gPoster(data[3].imageList, 360, 270);
    $('showSm3').getElementsByTagName('img')[0].src = gPoster(data[2].imageList, 250, 130);
    $('showSm2').getElementsByTagName('img')[0].src = gPoster(data[1].imageList, 250, 130);
    $('showSm1').getElementsByTagName('img')[0].src = gPoster(data[0].imageList, 250, 130);
    for (var i = 7; i >= 5; i--) {
        $('showBigPic').getElementsByTagName('img')[7 - i].src = gPoster(data[i].imageList, 780, 400);
        arr[7 - i] = data[i].titleFull;
...
var PortFun = {
    ...
    getSecondContent: function(liNum) { //一级栏目获取节目
        VOD_getAssetListByKeyword = {
            data: "<SearchAction assetId='" + nav[liNum] + "' startAt='1'  maxItems='12' portalId='" + portalId + "' client='" + cardId + "' account='" + userId + "' mergeTV='1'><UserParams sortButtons='byNewOld' sortDirection='DESC'><FilterBoxes serviceType='VOD'></FilterBoxes></UserParams></SearchAction>",
            callBack: getSearchTag
        };
        IEPG.getData(URL.VOD_getAssetListByKeyword, VOD_getAssetListByKeyword);
    },
    getFirstPage: function(liNum, startAt) {
        var start = startAt || 1;
        VOD_getAssetListByKeyword = {
            data: "<SearchAction assetId='" + nav[liNum] + "' startAt='" + start + "' maxItems='12' portalId='" + portalId + "' client='" + cardId + "' account='" + userId + "' mergeTV='1'><UserParams sortButtons='byNewOld' sortDirection='DESC'><FilterBoxes serviceType='VOD'></FilterBoxes></UserParams></SearchAction>",
            callBack: getSearchTag
        };
        IEPG.getData(URL.VOD_getAssetListByKeyword, VOD_getAssetListByKeyword);
    },
    getContent: function(liNum, tag, startAt) { //二级栏目获取节目
        var start = startAt || 1;
        VOD_getAssetListByKeyword = {
...
var Strategies = {
    nav: function(liNum, direction) {
        ...
        expect.setAttribute('style','display:none');
        allInit()
        ...
        switch (liNum) {
            ...
            case 3:
                {
                    expect.setAttribute('style','display:block');
                    navBar.setAttribute('style', 'height:110px');
                    navSecondUl.className = 'showHidden';
                    show.setAttribute("class", "showHidden");
                    change(currentName, { down: null });
                    secondShow.className = '';
                    secondShow.setAttribute("class", "showHidden");
                    firstNav = currentName;
                    if (direction == 'left' || direction == 'right')
                        PortFun.getSecondContent(liNum);
                    break;
    ...
    secondShowContent: function(liNum) {
        var Ul = $('secondShowContent'),
            ...
            title = li.getElementsByTagName('p')[0].innerHTML,
            arr = wordDismantle(firstNav),
            arrBp = wordDismantle(currentBp['myName']);
        allInit();
        clearClass('navSecondContent');
        img[liNum].setAttribute('class', 'secondPicChose');
        console.log([arrBp[0], titleBp])
        if (arrBp[0] == 'secondShowContent' && titleBp != null) {

            Ul.getElementsByTagName('li')[arrBp[1]].getElementsByTagName('p')[0].innerHTML = titleBp;
            //console.log('titleBp:',titleBp);
        }
            ...
            case 0:
                {
                    ...
                    for (var i = 0; i < 3; i++) {
                        if (li[i].className == 'showPic') num = 7 - i;
                    };
                    break;
                }
            case 1:
        ...
        if (liNum == 0) {
            ...
            switch (PortFun.buyNum) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    PortFun.reFreshBuy(6);
                    break;
                case 3:
                    PortFun.gotoWeixin(6);
                    break;
                case 4:
...
function btnAssert(currentBtn, direction) {
    ...
    if (wordDismantle(myName)[0] == 'secondShowContent') {
      ...
      if(getStyle(liBtn, 'display') == '' || getStyle(liBtn, 'display') == 'none'){
                                currentName = 'secondShowContent_0';
                                currentBtn = find(currentName);
                                setFocus(currentBtn);
        ...
        if (direction == 'left') {
            if (myName == 'secondShowContent_0' || myName == 'secondShowContent_6') {
                if (PortFun.curPage > 1) {
                    PortFun.pageLeft();
                    // setTimeout(function() {
                    //     var btnArr = wordDismantle(currentBtn['myName']);
                    //     var li = $('secondShowContent').getElementsByTagName('li')[btnArr[1]];
                    //     if (btnArr[0] === 'secondShowContent')
                    //         if (getStyle(li, 'display') == '' || getStyle(li, 'display') == 'none') {
                    //             currentName = 'secondShowContent_5';
                    //             currentBtn = find(currentName);
                    //             setFocus(currentBtn);
...
function doReturnKey() {
    var freezone_url = getGlobalVar("FREEZONE_URL");
    if (freezone_url) {
        setGlobalVar("FREEZONE_URL", "");
        window.location.href = freezone_url;
    } else {
